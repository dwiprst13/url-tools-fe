  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/qr?url=${encodeURIComponent(
          url
        )}&size=1000&color=${encodeURIComponent(
          color
      ...
      <div className="relative bg-gray-900/90 backdrop-blur-sm p-10 rounded-2xl shadow-2xl w-full max-w-xl border border-gray-800 z-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3">
            <label className="text-gray-300">Pilih Warna:</label>
            <input
              type="color"
              value={color}
              onChange={(e) => setColor(e.target.value)}
              className="w-12 h-12 p-1 rounded cursor-pointer border border-gray-700"
            />
        ...
        {qrBlobUrl && (
          <div className="mt-8 text-center bg-gray-800 p-4 rounded-xl border border-gray-700">
            ...
            <a
              href={qrBlobUrl}
              download={`${name || "qrcode"} generate with s.dprast.id.png`}
